...
$query1 = $conect->prepare('SELECT * from tbSaidaDoacao inner join tbFamilia on tbFamilia.idFamilia=tbSaidaDoacao.idFamilia inner join tbsaidaitems on tbsaidaitems.idSaidaDoacao=tbSaidaDoacao.idSaidaDoacao inner join tbItemDoacao on tbitemdoacao.idItemDoacao=tbsaidaitems.idItemDoacao inner join tbCategoriaDoacao on tbItemDoacao.idCategoriaDoacao=tbCategoriaDoacao.idCategoriaDoacao where ativaSaidaDoacao = 1 ');
$query1->execute();
...
$select = $select.' and tbSaidaDoacao.idFamilia = '.$familia;
...
//Armazena o comando e executa o comando.
//'SELECT chefeFamilia,rendaFamilia,idPrioridade, bairroFamilia, logradouroFamilia, numeroFamilia, complementoFamilia, referenciaFamilia, numeroTelefone from tbFamilia INNER JOIN tbIntegrantes ON tbIntegrantes.idFamilia=tbFamilia.idFamilia INNER JOIN tbTelefone ON tbTelefone.idFamilia=tbFamilia.idFamilia WHERE chefeFamilia like :1busca OR rgIntegrante = :2busca OR cpfIntegrante = :3busca OR nomeIntegrante like :4busca'
$query7 = $conect->prepare('SELECT * from tbItemdoacao');
$query7->execute();

$resultadoentra = $query7->fetchAll(PDO::FETCH_ASSOC);

$query3 = $conect->prepare('SELECT * from tbIntegrantes inner join tbFamilia on tbIntegrantes.idFamilia=tbFamilia.idFamilia where chefeIntegrante = 1 and ativaFamilia = 1 ORDER BY chefeFamilia ASC');
$query3->execute();
...
<div class="title">
<br><br><br>
<h2 style="font-weight: bold;color:black;">GERENCIAMENTO DE DOAÇÕES ENTREGUES</h2> 
<hr>
</div>
</div>
<div class="row">
...
<div class=" col-sm-5" >
<label for="buscanome" class=" form-label">Familia:</label>
<br>
<select name="familia" class="inputpesquisa form-select" class="form-select" id="familia">
<option value="">---</option>
<?php if(count($resultadofamilia)){ foreach($resultadofamilia as $familia){?>
<option value="<?php echo $familia['idFamilia']?>"><?php echo $familia['nomeIntegrante']?> - <?php echo $familia['cpfIntegrante']?></option>
<?php }}?>
</select>
</div>
<div class=" col-sm-3" >
...
<button type="submit"  class="button icon-add" style="padding-left:40px">Aplicar filtros</button>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
		<?php if(count($resultadoentra)){ foreach($resultadoentra as $entra){?>
        <form action="adicionar-doacaoentregue.php" method="post" style="padding:3%;">
		<?php }} ?>
...
<br>
<select list="familia" class="form-select" name="familia" id="familia">
...
<label>Item</label>
<br>
<select list="doacao" class="form-select" name="item" id="item">
<option value="">Selecione o item</option>
<?php if(count($resultadoentra)){ foreach($resultadoentra as $entra){?>
<option value="<?php echo $entra['idItemDoacao']?>"><?php echo $entra['descItemDoacao']?> </option>
<?php }}?>
</select>
</div>
<div class="col-sm-4">
...
<br>
<input type="date" id="data" class="defaultText form-control"  name="data">
</div>
</div>
<br>
            <br>
      ...
      <div class="modal-footer">
       ...
       <button type="submit"  style="padding-left:40px;" class="button icon-add"  >Finalizar doação</button>
      </div>
                  </form>
    </div>
  </div>
...
<button type="button"  class="button icon-add" style="padding-left:40px;" class="button icon-add" data-toggle="modal" id="modalbutton" data-target="#exampleModalCenter">Cadastrar nova doação</button> 
<a href="pesquisa-entredoacaoarquivos.php?value=0"><button type="button"  style="padding-left:40px;" class="button icon-archive">Ver doações arquivadas</button></a>
<br>
<br>
...
<td><?php echo $saida['descItemDoacao']?> </td> 
 <th > <a href="pdf-doacao.php?id=<?php echo $saida['idFamilia']?>&datasaidas=<?php echo $saida['dataSaidaDoacao']?>" target="_blank" style="text-decoration: none;">
<input type="button" class="button icon-pdf" style="padding-left:40px;"value="PDF"/>
</a></th>
<td><a href="arquivar-doacaoentregue.php?id=<?php echo $saida['idSaidaDoacao'] ?>"><button type="button"  style="padding-left:40px;" class="button icon-archive" >Arquivar</button></a>  </td>   
